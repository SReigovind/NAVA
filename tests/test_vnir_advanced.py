"""Advanced VNIR stress monitoring qualitative test.

Run from project root:  python tests/test_vnir_advanced.py
Run from tests/:        python test_vnir_advanced.py

All output paths are anchored to the directory containing this file,
so the script works correctly from either location.
"""
import base64
import glob
import os
import shutil
import sys
from pathlib import Path

# ── Path anchoring ───────────────────────────────────────────────────────────
TESTS_DIR    = Path(__file__).parent.resolve()
PROJECT_ROOT = TESTS_DIR.parent

if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))
if str(TESTS_DIR) not in sys.path:
    sys.path.insert(0, str(TESTS_DIR))

import requests
from test_utils import get_test_token, get_or_create_field, get_or_create_crop_plant, BASE_URL

# ── Output directories (all relative to tests/) ──────────────────────────────
IMAGE_OUT_DIR    = TESTS_DIR / "imageoutputs"
MARKDOWN_OUT_DIR = TESTS_DIR / "markdownoutput"


def decode_and_save_image(b64_str, filename):
    if not b64_str:
        return
    if b64_str.startswith("data:image"):
        b64_str = b64_str.split(",")[1]
    with open(filename, "wb") as f:
        f.write(base64.b64decode(b64_str))


def test_vnir_advanced():
    print("--- Starting Advanced VNIR Stress Monitoring Tests ---")
    IMAGE_OUT_DIR.mkdir(parents=True, exist_ok=True)
    MARKDOWN_OUT_DIR.mkdir(parents=True, exist_ok=True)
    report_path = MARKDOWN_OUT_DIR / "vnir_report.md"

    with open(report_path, "w") as md:
        md.write("# VNIR Stress Monitoring Qualitative Report\n\n")

        token = get_test_token()
        headers = {"Authorization": f"Bearer {token}"}
        field_id = get_or_create_field(token)
        crop_id, plant_id = get_or_create_crop_plant(token, field_id, "Banana")

        md.write("## Phase 1: Baseline Calibration (Banana)\n\n")
        md.write("Uploading 5 healthy banana images to establish the baseline.\n\n")
        md.write("| Scan | Status | Ratio | Baseline | Image | HSV Isolate | NIR Predicted |\n")
        md.write("|------|--------|-------|----------|-------|-------------|---------------|\n")

        base_dir = PROJECT_ROOT / "data" / "processed" / "efficientnet" / "test"
        healthy_imgs = (
            glob.glob(str(base_dir / "banana_healthy" / "*.jpg")) +
            glob.glob(str(base_dir / "banana_healthy" / "*.jpeg"))
        )[:5]

        for i, img_path in enumerate(healthy_imgs):
            with open(img_path, "rb") as f:
                res = requests.post(
                    f"{BASE_URL}/vnir-upload",
                    headers=headers,
                    data={"plant_id": plant_id, "crop_id": crop_id, "field_id": field_id},
                    files={"image": (os.path.basename(img_path), f, "image/jpeg")},
                )
            if res.status_code == 200:
                data    = res.json()
                hsv_b64 = data.get("hsv_image_base64")
                nir_b64 = data.get("vnir_image_base64")

                base_name = f"vnir_calib_{i}"
                shutil.copy(img_path, IMAGE_OUT_DIR / f"{base_name}_orig.jpg")
                if hsv_b64:
                    decode_and_save_image(hsv_b64, IMAGE_OUT_DIR / f"{base_name}_hsv.jpg")
                if nir_b64:
                    decode_and_save_image(nir_b64, IMAGE_OUT_DIR / f"{base_name}_nir.jpg")

                # Paths relative to the markdown file (markdownoutput/ → ../imageoutputs/)
                orig_rel = f"../imageoutputs/{base_name}_orig.jpg"
                hsv_rel  = f"../imageoutputs/{base_name}_hsv.jpg"
                nir_rel  = f"../imageoutputs/{base_name}_nir.jpg"

                md.write(
                    f"| {i+1} | {data.get('status')} | {data.get('ratio'):.4f} | "
                    f"{data.get('baseline') or 'N/A'} | "
                    f"<img src='{orig_rel}' width='100'/> | "
                    f"<img src='{hsv_rel}' width='100'/> | "
                    f"<img src='{nir_rel}' width='100'/> |\n"
                )
                print(f"  Calib {i+1} -> {data.get('status')} (Ratio: {data.get('ratio'):.4f})")

        md.write("\n<div style='page-break-before: always;'></div>\n\n")
        md.write("## Phase 2: Stress Detection (Banana Sigatoka)\n\n")
        md.write("Testing with 3 diseased (Sigatoka) images.\n\n")
        md.write("| Scan | Status | Ratio | Vs Baseline | Image | HSV Isolate | NIR Predicted |\n")
        md.write("|------|--------|-------|-------------|-------|-------------|---------------|\n")

        sigatoka_imgs = (
            glob.glob(str(base_dir / "banana_sigatoka" / "*.jpg")) +
            glob.glob(str(base_dir / "banana_sigatoka" / "*.jpeg"))
        )[:3]

        for i, img_path in enumerate(sigatoka_imgs):
            with open(img_path, "rb") as f:
                res = requests.post(
                    f"{BASE_URL}/vnir-upload",
                    headers=headers,
                    data={"plant_id": plant_id, "crop_id": crop_id, "field_id": field_id},
                    files={"image": (os.path.basename(img_path), f, "image/jpeg")},
                )
            if res.status_code == 200:
                data    = res.json()
                hsv_b64 = data.get("hsv_image_base64")
                nir_b64 = data.get("vnir_image_base64")

                base_name = f"vnir_stress_{i}"
                shutil.copy(img_path, IMAGE_OUT_DIR / f"{base_name}_orig.jpg")
                if hsv_b64:
                    decode_and_save_image(hsv_b64, IMAGE_OUT_DIR / f"{base_name}_hsv.jpg")
                if nir_b64:
                    decode_and_save_image(nir_b64, IMAGE_OUT_DIR / f"{base_name}_nir.jpg")

                orig_rel = f"../imageoutputs/{base_name}_orig.jpg"
                hsv_rel  = f"../imageoutputs/{base_name}_hsv.jpg"
                nir_rel  = f"../imageoutputs/{base_name}_nir.jpg"

                vs_b     = data.get("vs_baseline")
                vs_b_str = f"{vs_b:.2f}%" if vs_b is not None else "N/A"

                md.write(
                    f"| {i+1} | **{data.get('status')}** | {data.get('ratio'):.4f} | {vs_b_str} | "
                    f"<img src='{orig_rel}' width='100'/> | "
                    f"<img src='{hsv_rel}' width='100'/> | "
                    f"<img src='{nir_rel}' width='100'/> |\n"
                )
                print(f"  Stress {i+1} -> {data.get('status')} (Ratio: {data.get('ratio'):.4f}, vs: {vs_b_str})")

    print(f"Report saved to {report_path}")


if __name__ == "__main__":
    test_vnir_advanced()
